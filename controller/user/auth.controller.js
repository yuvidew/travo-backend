...
const otpCheck = async (req, res) => {
    ...
    try {
        ...
        await db.query("UPDATE users SET otp_code = NULL, otp_expiry = NULL WHERE email = ?", [email]);
...
const googleAuth = async (req, res) => {
    ...
    try {
        ...
        if (rows.length === 0) {
            ...
            await db.query("INSERT INTO users (google_id , name , email , picture) VALUES (?, ?, ?, ?)", [id, name, email, picture]);
